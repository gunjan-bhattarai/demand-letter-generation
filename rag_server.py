...
# --- Configuration ---
# Read from environment variables, with defaults from your original script
...
if not TOGETHER_API_KEY:
    log_message("Error: TOGETHER_API_KEY environment variable not set.")
    sys.exit(1)
...
if EMBEDDING_PROVIDER == "huggingface":
    ...
    log_message(f"Using Together AI embedding model: {TOGETHER_EMBEDDING_MODEL}...")
    test_response = client.embeddings.create(model=TOGETHER_EMBEDDING_MODEL, input=["Test"])
    embedding_dimension = len(test_response.data[0].embedding)
log_message(f"Embedding dimension: {embedding_dimension}")

# --- 2. Load and Chunk Documents (Only if index doesn't exist) ---
def load_and_chunk_documents(data_dir, chunk_size, chunk_overlap):
    ...
    for filename in os.listdir(data_dir):
        if filename.endswith(".txt"): # Note: You can adapt this for PDFs
...
if not os.path.exists(FAISS_INDEX_PATH):
    ...
    if EMBEDDING_PROVIDER == "huggingface":
        ...
        for i in range(0, len(docs_to_index), batch_size):
            response = client.embeddings.create(model=TOGETHER_EMBEDDING_MODEL, input=docs_to_index[i:i + batch_size])
            document_embeddings.extend([item.embedding for item in sorted(response.data, key=lambda x: x.index)])
...
def answer_question_with_rag(query, faiss_index, doc_store, faiss_id_map, k_retrieval=5):
    if EMBEDDING_PROVIDER == "huggingface":
        ...
        response = client.embeddings.create(model=TOGETHER_EMBEDDING_MODEL, input=[query])
    ...
    messages = [
        {"role": "system", "content": "You are a helpful assistant. Use the following context to answer the question. If the answer is not in the context, say 'I don't know'. Do not tell the user you are referencing context."},
...
def send_error(request_id, code, message):
    ...
    # JSON-RPC spec allows null id for parse errors, but some clients expect a valid id
    # Use 0 as a fallback for better compatibility
    error_response = {
        "jsonrpc": "2.0",
...
log_message("🚀 RAG server is starting. Ready for MCP requests.")
...
while True:
    try:
        ...
        if method == "initialize":
            ...
            # Client confirms initialization is complete - this is a notification, no response needed
                    ...
                    "tools": [
                        {
                            ...
                            "description": "Search and query legal documents using RAG (Retrieval-Augmented Generation)",
                            "inputSchema": {
                                "type": "object",
                                "properties": {
                                    "query": {
        ...
        # For parse errors, we can't extract request_id, so use null
        send_error(None, -32700, "Parse error")
    except Exception as e:
        log_message(f"An unexpected error occurred: {e}")
        # Try to get request_id if it was parsed, otherwise use null
        try:
            req_id = request_id
        except NameError:
            req_id = None
